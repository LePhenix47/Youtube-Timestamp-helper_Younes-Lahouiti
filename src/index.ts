  ...
  try {
    ...
    const chaptersForSync = parseResult.chapters!.map((chapter, index) => {
      ...
      const endTime = nextChapter
        ? nextChapter.start
        : videoDuration || chapter.start + 60;
    ...
    for (
      let i = 0;
      i < Math.min(importedChapters.length, parsedChapters.length);
      i++
    ) {
      ...
      const titleHeading = chapter.element.querySelector<HTMLHeadingElement>(
...
// Add event listener to timestamp chapter button for scrolling to chapter in sidebar
timeStampControlsChapter?.addEventListener("click", () => {
  const chapterId = timeStampControlsChapter.dataset.chapterId;
  if (!chapterId) return;
  ...
  for (const chapterElement of chapterElements) {
    if (chapterElement.dataset.chapterId !== chapterId) {
      continue;
    }
    // Scroll to the chapter with smooth behavior, centered to avoid sticky header
    chapterElement.scrollIntoView({ behavior: "smooth", block: "center" });

    // Add highlight class and remove it when animation ends
    chapterElement.classList.add("chapter-highlighted");
    chapterElement.addEventListener(
      "animationend",
      () => {
        chapterElement.classList.remove("chapter-highlighted");
      },
      { once: true }
    );
